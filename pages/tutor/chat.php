...
<?php
// Получаем обучающихся, с которыми есть принятые заявки или уже есть переписка
...
$contacts = $db->fetchAll(
  "SELECT DISTINCT u.user_id, u.full_name
   ...
   WHERE u.user_type = 'student' AND (
     u.user_id IN (
       SELECT tr.student_id FROM tutor_requests tr
       WHERE tr.tutor_id = ? AND tr.status IN ('accepted','completed')
     )
     OR u.user_id IN (
...
<div class="chat-wrapper">
  <div class="chat-contacts">
    <div class="chat-contacts-header">Обучающиеся</div>
    <?php if (empty($contacts)): ?>
      <div class="no-contacts">Нет доступных собеседников. Примите заявку от обучающегося.</div>
    <?php else: ?>
  ...
  <div class="chat-area">
    ...
    <div class="chat-messages" id="chatMessages">
      <div class="chat-empty">Выберите обучающегося для начала общения</div>
    </div>
